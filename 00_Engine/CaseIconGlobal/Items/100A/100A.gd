...
@export var current_state : GlobalState.CaseState
...
func _physics_process(delta: float) -> void:
	if current_state == GlobalState.CaseState.HAND:
		_hand_state()
	else:
		_drop_state(delta)
...
func _cal_Attribute() -> void:
	realCoolTime = OriginalCoolTime / Player.instance.allCoolTime_Mag

func _player_detect() -> void:
	#物品拾取范围(局部函数)
	if drop_thing.instance != null and Player.instance != null:
		var tempRange: int = drop_thing.instance.take_range
		if !Player.instance.global_position.x < global_position.x + tempRange:
			return
		if !Player.instance.global_position.x > global_position.x - tempRange:
			return
		if !Player.instance.global_position.y < global_position.y + tempRange:
			return 
		if Player.instance.global_position.y > global_position.y - tempRange:
			if Input.is_action_just_pressed("take_drop") and current_state != GlobalState.CaseState.HAND:
				PlayerUI.instance.BackPackNode.BackPackWindows.check_empty(id_dic[gunType])
				queue_free()

	if current_state == GlobalState.CaseState.HAND:
		if Input.is_action_just_pressed("drop"):
			current_state = GlobalState.CaseState.DROP
