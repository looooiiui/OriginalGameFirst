...
@export var MonsterAnim : AnimatedSprite2D
...
@export var perGlobalPosition : Vector2
#死亡管理
...
func _ready() -> void:
	...
	#受伤颜色管理初始化
	originalColor = MonsterAnim.modulate
	#初始化动画
...
func _process(delta: float) -> void:
	...
	#------受伤管理------#
	MonsterAnim.modulate = lerp(MonsterAnim.modulate, originalColor, color_Smooth)
	
	MonsterAnim.global_position = global_position
...
func MonsterAnimPlay():
	if rotation > PI / 4 or rotation < -PI / 4:
		is_Back = true
	else:
		is_Back = false
	
	if !is_Back:
		MonsterAnim.flip_h = true
	elif is_Back:
